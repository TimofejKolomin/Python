...
class Drop:
    ...
    def move(self):
        if drop.wait_count < drop.timeout: # Не время перемещать
            drop.wait_count += 1 # Увеличиваем счетчик паузы
            return False
        else: #Уже можно перемещать
            drop.wait_count = 0 # Сброс счетчика паузы
            drop.y += 1 # Перемещаем каплю/антикапю вниз
            return True
...
bext.title('Matrix') # Смена названия консольного окна
bext.clear() # Очистка консольного окна
bext.hide() # Скрываем курсор в окне
width, height = bext.size() # Высота и ширина консольного окна
width -= 1
height -= 1
...
while True:
    for drop in drops:
        if drop.move(): # Проверка перемещения элемента
            drop.draw() # Рисуем элемент
            if drop.y >= height: # Достиг конца
                drop.renew() # Обновляем элемент
    key = bext.getKey(blocking = False) # Проверям нажата ли клавиша
    if key == 'esc': #! Если нажата ESC, то выхдим из консоли
        bext.clear()
        sys.exit()
